...
export const createUser = async (req, res) => {
    const { name, email, mobile, password, group_id, confirm_password } = req.body;
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
        INSERT INTO users (name, email, mobile, group_id, password)
        VALUES (@name, @email, @mobile, @group_id, @password);

        SELECT id, name, email, mobile, group_id, status
        FROM users
...
export const updateUserById = async (req, res) => {
    const { id } = req.params;
    const { name, email, mobile, password, status, group_id } = req.body;
    try {
        const pool = await connectToDatabase(); // Use the function to get the connection pool
        ...
        handleResponse(res, 200, { id, name: trimmedName, email: trimmedEmail, mobile: trimmedMobile, status: trimmedStatus, group_id: trimmedGroupId });
    } catch (error) {
        handleResponse(res, 400, error.message, false);
    }
};
